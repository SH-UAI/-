...
app.controller("baseController",function ($scope) {
    ...
    $scope.updateIds = function ($event,id) {
        // 如果单选全部选中，把全选的状态选中
        // $("#selall").prop('checked',$('.selectOne:checked').length == $scope.paginationConf.itemsPerPage);
        if($('.selectOne:checked').length == $scope.paginationConf.itemsPerPage){
    ...
    // 模板在显示的时候  是json字符串   需要把json转换成字符串然后拼接
    ...
    // 从集合中的key获取value
    //[{"attributeName":"chima","attributeValue":["165","170"],"$$hashKey":"00S"},{"attributeName":"sanwei","attributeValue":["22","19"],"$$hashKey":"013"}]
    // 根据上面的json格式，key是attributeName  valeu是  chima
