...
public class ItemCatController {
	...
	public List<TbItemCat> findItemCatByPid(long parentId){
			return itemCatService.findItemCatByPid(parentId);
	}

	// 删除分类
	@RequestMapping("deleteType")
	public Result deleteType(long [] ids){

		try {
			itemCatService.deleteType(ids);
			return new Result(true, "删除成功");
		} catch (Exception e) {
			e.printStackTrace();
			return new Result(false, "删除失败");
